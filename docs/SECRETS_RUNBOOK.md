...
| **`NPM_TOKEN`** (publishing the packages) | An npm **automation** token used by the release workflow to publish `@caucus/*`. Relevant only to whoever owns the npm publishing pipeline — not to a backbone operator. | GitHub Actions secret store; the publish runner only. Never on a backbone host. | A leaked publish token lets an attacker push malicious `@caucus/*` versions to npm (supply-chain). Use an **automation** token (bypasses 2FA), scope it to the org, and rotate via npm → Access Tokens. The publish workflow (`.github/workflows/release.yml`) is gated on this secret and is a safe no-op without it — see [`RELEASING.md`](RELEASING.md). |
...
| **Write rejected (401)** | **HTTP response only.** An unknown/absent bearer gets an identical `401` (no oracle). | **GAP:** the server does **not log** *write*-auth failures. There is no auth-failure counter, no audit line, nothing on stderr for the write routes. You can only observe 401s from a proxy/tunnel access log if you put one in front. |
| **Control-plane op (mint/revoke/rotate)** | **Stderr audit line (CAU-128).** Each mint/revoke/rotate — success **or** a failed attempt against an *enabled* control surface — writes one `{"kind":"caucus.admin.audit", op, agent_id, owner, digest, ts, result}` line to **stderr** (default ON; `CAUCUS_ADMIN_AUDIT=0` disables). Digest-only, never the token or the admin credential (ADR-C12); stderr only, never the channel log (ADR-C6). | **Capture stderr** and alert on `result:"unauthorized"` (a rogue admin attempt) and on unexpected mints. When the control surface is *disabled* (`CAUCUS_ADMIN_TOKEN` unset) nothing is emitted — the ops can't happen. The trail is ephemeral (in-stderr); ship it to durable storage if you need retention. |
