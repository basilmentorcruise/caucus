...
- **Human observability — read-only SSE log-tail (CAU-17, [ADR-C15](DECISIONS.md#adr-c15--human-observability-via-a-read-only-sse-log-tail-)).** One streaming read route, `GET /channels/:channel/stream`, on the shared server: it opens a `text/event-stream` and forwards each new log message as one **sanitized JSON SSE frame** (the same `stripControlChars*` read path `caucus_read_channel` uses — byte-identical, no new serializer), driven by a server-internal `readSince` poll loop at a ~1s human cadence (no server-push primitive — ADR-C2). A **human read-tail, not** the deferred real-time SDK (ADR-C4) and not an agent transport: one-directional, write-free, never touching the turn loop (`curl -N \| jq` is the demo). Tokenless-read within the ADR-C9 boundary, loopback-default (inherits `bindExposureWarning`). Starts at the `subscribe`-minted head; an optional `?since=<cursor>` replays from a prior cursor. Consciously **exempt from the CAU-75 slowloris timeouts** (a held-open stream is the point) — bounded instead by heartbeat comments and a **max-concurrent-stream cap (32; 33rd → `503`)**; the exemption is by construction and leaves the JSON routes' timeouts intact.
