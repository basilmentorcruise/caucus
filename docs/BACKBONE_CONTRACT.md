...
The backbone owns the **log + claim ledger + cursors + seatbelts** (per-agent rate limit + loop/dup detection, ADR-C8 / CAU-8 — see below) and, as of CAU-18, the **claim lifecycle** (lease expiry / heartbeat / reassignment / done — see *Claim lifecycle* below). It does NOT define the message shape (that is [`@caucus/schema`](MESSAGE_SCHEMA.md)) and it does NOT do identity anchoring (CAU-9/13 — the caller anchors `agent_id`/`owner` and the backbone treats them as trusted input).
...
## Claim lifecycle (expiry, heartbeat, reassignment, done) (CAU-18)

CAU-18 turns the inert `lease_ttl`/`heartbeat` schema fields into enforced behaviour and adds two ledger transitions, all **inside the existing ADRs** — no new wire message type, no new `status` enum value, no schema field, **no schema-version bump**. Each transition is expressed with an existing `claim`-typed message.

Three things back this: the `ClaimRecord` now also stores `claimed_at_ms` (the injected-clock instant the lease was last granted/renewed/reassigned) and `lease_ttl_s` (the lease length in seconds, internal projection state — never on the wire); a single shared synchronous helper holds the one no-`await` critical region for `claim`/`reassign`/`done`; and the wall-clock comes from the injectable `#seatbelt.now()`.

- **Lease & expiry (lazy, wall-clock).** A `claim` with `lease_ttl: T` is granted normally and records `claimed_at_ms = now()`. The lease **lapses** once `now() >= claimed_at_ms + T*1000`. Expiry is **lazy**: it is evaluated **inside the next `claim`/`reassign`/`done` on that target**, never by a timer or background sweeper (there is no `setTimeout`/`setInterval` anywhere in the backbone — guarded by a test). A lapsed lease **frees** the target — the very next `claim` is `granted` and **OVERWRITES** the stale ledger entry (it is never left dangling). A claim with **no `lease_ttl` never expires** (backward-compatible with the original hold-forever first-write-wins). Expiry posts **nothing** — it is silent until the freeing claim arrives.
- **Clock-backwards is safe.** Lease math is a single `>=` comparison against the one `now()` read for the operation. If the (injected or system) clock moves **backwards** so `now() < claimed_at_ms`, the comparison is false and the lease is treated as **still held** — never spuriously expired.
- **Heartbeat (holder-only renew).** A `claim` with `heartbeat: true` from the **current holder** — matched on the anchored `owner` (ADR-C7) — resets `claimed_at_ms` to `now()` (moving the deadline) and is reported as `granted`; the renew is a visible `claim` message. A `heartbeat` from a **different owner** does **not** renew or steal — it is an ordinary `already_claimed` loss, deadline unchanged. A `heartbeat` on an **unheld / lapsed / never-claimed** key is just a fresh claim (`granted`, new lease).
- **Reassignment (`reassignClaim`).** The **current holder** (matched on the anchored `owner`, ADR-C7 — the human, not the session `agent_id`, so a human may reassign across their own sessions) hands a **live** target to a new holder (`ClaimAssignee`). On success a `claim` message authored by the **authenticated holder** (announcing the handoff) is appended in the same atomic step and the **ledger** is overwritten to point at the assignee; the result is `granted`. The assignee identity is **poster-asserted data the authenticated holder vouches for** (like a `to[]` recipient) — it is **never identity-anchored** (the HTTP edge anchors only the authorizer). A reassign by a **different owner** loses (`already_claimed`, ledger untouched). A reassign of an **unheld/lapsed** target is **not** privileged — an expired holder has no special right — so it degrades to a plain fresh grant to the assignee.
- **Done-state (`markClaimDone`).** The current holder (owner-matched) marks a **live** target finished: a `claim` message carrying `status:"resolved"` is appended (a **visible** completion record — unlike a silent lapse) and the **ledger entry is deleted**, freeing the target so a later `claim` starts a **fresh** lease. Done by a **different owner** is a no-op `already_claimed` (you cannot close someone else's claim); done on an **unheld/lapsed/never-claimed** target is a no-op `not_held` (an expired holder closing its own lapsed claim posts nothing). The `delete` happens inside the same no-`await` critical region as the read/write.
- **Result shape.** Every successful transition reuses `outcome: "granted"` (the appended `message` already distinguishes a fresh claim from a renew/reassign/resolved). `markClaimDone` adds one TS-only `ClaimResult` variant, `outcome: "not_held"` (a no-op with no holder to name). These are TS contract types, **not** schema/wire changes. All result/error strings stay **value-free** (ADR-C12): the only identity bytes returned are the existing four-field `already_claimed.by`.
- **No-`await` reaffirmation.** `claim`, `reassignClaim`, and `markClaimDone` all funnel through one shared synchronous helper that reads the single `now()` value **once** before the marked region and does every ledger read/expiry-check/write/delete with **no `await` between** — so the whole-ballgame CAS invariant (above) covers the lifecycle transitions too. When durability lands, this stays a single transaction / unique-constraint upsert (the `done` delete is part of that same transaction).
...
No SQLite durability, no identity anchoring (CAU-9 — done by the caller / HTTP edge). Lease/heartbeat enforcement and the reassignment / done transitions HAVE landed (CAU-18 — see *Claim lifecycle* above); lifecycle state is in-memory only (it lives as long as the process, per ADR-C2/C14).
