...
4. **Build** — match the surrounding code's style; add tests; update docs if behavior changes. **New workspace package?** It must carry a `tsconfig.test.json` (copy one from an existing package) — `pnpm typecheck` runs `tsc -p tsconfig.test.json` in every package under `packages/` and fails with TS5058 if it's missing (plain-JS workspaces like `examples/*` sit outside the TS gate).
